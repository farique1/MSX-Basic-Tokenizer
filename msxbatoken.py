...
Convert ASCII MSX Basic programs to tokenized format

Copyright (c) 2019 - Fred Rique.
https://github.com/farique1/MSX-Basic-Tokenizer

NEW: .mlt Assembler like List file export option
...
NOTE: Known discrepancies:
        ...
        Spaces at the end of a line (verified with PRINT and REM) are removed
          The MSX does not remove it if loading from an .asc file (it is removed if typed on the machine)
      ...
      Tested with over 100 random basic programs from magazines and other sources, some crated to stress the conversions
        However there still should be some (several?) fringe cases not covered here. Be careful.
'''
...
file_load = 'basictests/pi.asc'  # Source file
file_save = 'basictests/pi.bto'  # Destination file
bytes_width = 16
save_list = True
verbose_level = 1
is_from_build = False
...
width_line = int(bytes_width * 3) + 7
now = datetime.now()
...
parser.add_argument("-bw", default=bytes_width, type=int, choices=range(1, 32), help="The amount of bytes shown per line")
parser.add_argument("-sl", default=save_list, action='store_true', help="Save a .mlt list file detailing the tokenization")
...
def show_log(line_number, text, level, **kwargs):
    ...
    line_number = str(
        line_number) if line_number == '' else '(' + str(line_number) + '): '
...
for line_source in ascii_code:
    if ord(line_source[0]) < 48 or ord(line_source[0]) > 57:
        if ord(line_source[0]) == 26:  # Avoid '' on last line of some listings
            ...
            show_log(line_number, ' '.join(['line_not_starting_with_number']), 1, bullet=2)
            show_log('', 'Execution_stoped\n', 1, bullet=0)
            raise SystemExit(0)
